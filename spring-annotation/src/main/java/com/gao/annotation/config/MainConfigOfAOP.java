...
import com.gao.annotation.aop.LogAspects;
 ...
 *
 * AOP原理:【看给容器中注册了什么组件，这个组件什么时候工作，这个组件的功能时什么？】
 *      @EnableAspectJAutoProxy:
 * 1、@EnableAspectJAutoProxy:
 *      @Import(AspectJAutoProxyRegistrar.class): 给容器中导入AspectJAutoProxyRegistrar
 *          利用AspectJAutoProxyRegistrar自定义给容器中注册bean；BeanDefinetion
 *          internalAutoProxyCreator=AnnotationAwareAspectJAutoProxyCreator
 *      给容器中注册一个AnnotationAwareAspectJAutoProxyCreator(AspectJ自动代理的创建器)
 * 2、AnnotationAwareAspectJAutoProxyCreator:
 *      AnnotationAwareAspectJAutoProxyCreator ->(父类) AspectJAwareAdvisorAutoProxyCreator -> AbstractAdvisorAutoProxyCreator
 ...
 *                      4.2.2.1、resolveBeforeInstantiation(beanName, mbdToUse);解析BeanInstantiation
 ...
 *      2.2、判断当前bean是否是基础类型的advice、Pointcut、Advisor、AopInfrastructureBean，或者是否是切面(@Aspect)
 *      2.3、是否需要跳过
 *          2.3.1、获取候选的增强器(切面里面的通知方法)【List<Advisor> candidateAdvisors】
 ...
 *      3.2、根据ProxyFactory对象获取将要执行的目标方法拦截器链接；
 ...
 *              5.2.1、得到目标方法的拦截器(增强器包装成拦截器MethodInterceptor)
